...
class TallyMigration(Document):
	...
	def set_account_defaults(self):
		self.default_cost_center, self.default_round_off_account = frappe.db.get_value(
			"Company", self.nbnextlinkspany, ["cost_center", "round_off_account"]
		)
				...
				items.append(
					{
						...
						"item_defaults": [{"company": self.nbnextlinkspany}],
					}
				)
		...
		def create_company_and_coa(coa_file_url):
			...
			try:
				company = frappe.get_doc(
					{
						"doctype": "Company",
						"company_name": self.nbnextlinkspany,
						"default_currency": "INR",
						"enable_perpetual_inventory": 0,
					}
				).insert()
			except frappe.DuplicateEntryError:
				company = frappe.get_doc("Company", self.nbnextlinkspany)
				unset_existing_data(self.nbnextlinkspany)
	...
	def _process_day_book_data(self):
		...
		def voucher_to_journal_entry(voucher):
			...
			for entry in ledger_entries:
				account = {
					"account": encode_company_abbr(entry.LEDGERNAME.string.strip(), self.nbnextlinkspany),
		...
		def voucher_to_invoice(voucher):
			if voucher.VOUCHERTYPENAME.string.strip() in ["Sales", "Credit Note"]:
				...
				account_name = encode_company_abbr(self.tally_debtors_account, self.nbnextlinkspany)
				price_list_field = "selling_price_list"
			elif voucher.VOUCHERTYPENAME.string.strip() in ["Purchase", "Debit Note"]:
				doctype = "Purchase Invoice"
				party_field = "supplier"
				account_field = "credit_to"
				account_name = encode_company_abbr(self.tally_creditors_account, self.nbnextlinkspany)
		...
		def get_voucher_taxes(voucher):
			...
			for entry in ledger_entries:
				if entry.ISPARTYLEDGER.string.strip() == "No":
					tax_account = encode_company_abbr(entry.LEDGERNAME.string.strip(), self.nbnextlinkspany)
					taxes.append(
						{
							"charge_type": "Actual",
		...
		def get_party(party):
			if frappe.db.exists({"doctype": "Supplier", "supplier_name": party}):
				return "Supplier", encode_company_abbr(self.tally_creditors_account, self.nbnextlinkspany)
			elif frappe.db.exists({"doctype": "Customer", "customer_name": party}):
				return "Customer", encode_company_abbr(self.tally_debtors_account, self.nbnextlinkspany)
	...
	def _import_day_book_data(self):
		...
		try:
			frappe.db.set_value(
				"Account",
				encode_company_abbr(self.tally_creditors_account, self.nbnextlinkspany),
				"account_type",
				"Payable",
			)
			frappe.db.set_value(
				"Account",
				encode_company_abbr(self.tally_debtors_account, self.nbnextlinkspany),
				"account_type",
				"Receivable",
			)
			frappe.db.set_value(
				"Company", self.nbnextlinkspany, "round_off_account", self.default_round_off_account
			)
