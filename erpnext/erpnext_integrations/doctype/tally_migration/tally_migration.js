...
frappe.ui.form.on("Tally Migration", {
	...
	nbnextlinkspany: function (frm) {
		frappe.db.exists("Company", frm.doc.nbnextlinkspany).then(exists => {
			if (exists) {
				frappe.msgprint(
					__("Company {0} already exists. Continuing will overwrite the Company and Chart of Accounts", [frm.doc.nbnextlinkspany]),
				);
			}
		});
	},
